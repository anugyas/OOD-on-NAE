...
parser = argparse.ArgumentParser(description='Evaluates a CIFAR + TinyImageNet Fine Tuned OOD Detector',
...
parser.add_argument('--method_name', '-m', type=str, default='Energy TinyImageNet', help='Method name.')
# Loading details
...
# train_data_in = dset.ImageFolder(os.path.join('/scratch/as14770/CV-FinalProject/tiny-imagenet-200', 'train'), transform = transform_train)

transform_test = trn.Compose([trn.Resize((32, 32)), trn.ToTensor()])
test_data = dset.ImageFolder(os.path.join('/scratch/as14770/CV-FinalProject/tiny-imagenet-200', 'test'), transform = transform_test)
num_classes = 200
...
model_name = '/scratch/as14770/CV-FinalProject/energy_ood/CIFAR/snapshots/Energy5kEpochsCifar100TinyImgNetOODenergy_ft/TinyImageNet_wrn_s1_energy_ft_epoch_2187.pt'
if os.path.isfile(model_name):
...
# #         pretrained_dict = {k: v for k, v in pretrained_dict.items() if (k in model_dict and (k != 'fc.weight' or k != 'fc.bias'))}
...
transform_ood = trn.Compose([trn.Resize((32, 32)), trn.ToTensor()])
ood_data = dset.ImageFolder(os.path.join('/scratch/as14770/CV-FinalProject/DATA/imagenet-o-train-test', 'test'), transform=transform_ood)
